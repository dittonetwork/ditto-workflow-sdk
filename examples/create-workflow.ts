import { WorkflowBuilder, JobBuilder, ChainId, submitWorkflow } from "../src";
...
const IPFS_SERVICE_URL =
  process.env.IPFS_SERVICE_URL || "https://ipfs-service.dittonetwork.io";
const WORKFLOW_CONTRACT_ADDRESS = process.env
  .WORKFLOW_CONTRACT_ADDRESS as `0x${string}`;
...
async function createAndSubmitWorkflow(
  ...
  const workflow = WorkflowBuilder.create(
    addressToEmptyAccount(ownerAccount.address!),
  )
    .addCronTrigger("*/2 * * * *")
    .setCount(3)
...
async function main() {
  try {
    ...
    const response = await createAndSubmitWorkflow(
      ownerAccount,
      executorAddress,
      storage,
    );
    ...
    logger.info(
      "📋 Transaction Hashes:",
      response.userOpHashes
        .map((op) => op.receipt?.transactionHash)
        .filter(Boolean),
    );
