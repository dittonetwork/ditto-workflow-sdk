import { executeFromIpfs } from "../src";
...
async function main() {
  try {
    ...
    const ipfsHash =
      process.argv[2] || "QmbRtCvtT3yXf8a5Bqs3NgSi2WLWJjUWBSYg2xCnKuNqXq";

    if (!EXECUTOR_PRIVATE_KEY) {
      throw new Error(
        "Missing EXECUTOR_PRIVATE_KEY in environment variables. Please check your .env file.",
      );
    }
    ...
    console.log(
      `${process.env.VITE_STATUS_DASHBOARD_URI}/workflow/status/${ipfsHash}`,
    );
  } catch (error) {
